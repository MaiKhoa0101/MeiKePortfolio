...
<template>
  <div class="bg-background min-h-screen">
    ...
    <div class="fixed z-[999] inset-0 pointer-events-none flex items-end justify-end p-6 md:p-10">
      ...
      <div 
        class="relative transition-all duration-700 cubic-bezier(0.34, 1.56, 0.64, 1) pointer-events-auto"
      ...
      >
        <div 
          @click="!isProfileOpen ? toggleProfile() : null"
        ...
        >
          ...
          <div v-if="!isProfileOpen" class="w-full h-full flex items-center justify-center relative group">
             <img src="https://placehold.co/100x100/1e1b4b/white?text=Me" class="w-full h-full object-cover opacity-90 group-hover:opacity-100" />
          </div>

          <div v-else class="w-full h-full flex flex-col md:flex-row relative">
             <button @click.stop="toggleProfile" class="absolute top-5 right-5 z-20 p-2 bg-white/5 hover:bg-red-500/80 text-white rounded-full transition-colors backdrop-blur-md">
                <Icon name="heroicons:x-mark" size="24" />
             </button>

             <div class="md:w-5/12 h-64 md:h-full relative">
                <img src="https://placehold.co/600x800/1e1b4b/white?text=Profile" class="absolute inset-0 w-full h-full object-cover" />
             ...
             <div class="md:w-7/12 p-8 md:p-12 overflow-y-auto custom-scrollbar">
                <h2 class="text-3xl font-bold mb-6 bg-gradient-to-r from-white to-primary/50 bg-clip-text text-transparent">
                  Về bản thân
                </h2>
                <p class="text-surface-content/80 mb-8 leading-loose text-lg font-light">
                  Tôi là một lập trình viên đam mê tạo ra các sản phẩm web có tính tương tác cao. Tôi tin rằng một website tốt không chỉ chạy đúng logic mà còn phải mang lại cảm xúc cho người dùng.
                </p>
                ...
                <div class="flex flex-wrap gap-2 mb-10">
                  <span v-for="skill in ['Vue/Nuxt', 'NodeJS', 'Tailwind', 'UI/UX', 'Figma']" :key="skill" class="px-4 py-2 bg-white/5 border border-white/10 text-white rounded-xl text-sm hover:border-primary/50 transition-colors cursor-default">
                    {{ skill }}
                  </span>
                </div>

                <div class="grid grid-cols-2 gap-4">
                  <a href="#" target="_blank" class="group flex items-center gap-4 p-4 bg-background border border-border rounded-2xl hover:border-primary/50 transition-all hover:-translate-y-1">
                    <div class="p-3 bg-white/5 rounded-full group-hover:bg-primary group-hover:text-white transition-colors text-surface-muted">
                  ...
                  <a href="#" class="group flex items-center gap-4 p-4 bg-background border border-border rounded-2xl hover:border-primary/50 transition-all hover:-translate-y-1">
                    <div class="p-3 bg-white/5 rounded-full group-hover:bg-primary group-hover:text-white transition-colors text-surface-muted">
    ...
    <section class="h-screen relative overflow-hidden">
      <Swiper
        ...
        class="h-full w-full"
      >
        <SwiperSlide v-for="project in featuredProjects" :key="project.id" class="relative bg-background">
          <img 
            :src="project.image" 
            class="absolute inset-0 w-full h-full object-cover object-center md:object-right opacity-60 mix-blend-overlay" 
          />
          
          <div class="absolute inset-0 bg-gradient-to-r from-background via-background/95 to-transparent z-0"></div>
          <div class="absolute inset-0 bg-gradient-to-t from-background via-transparent to-transparent z-0"></div>

          <div class="relative z-10 container mx-auto px-6 h-full flex items-center">
            <div class="max-w-3xl pt-24 md:pt-0">
              
              <div class="inline-flex items-center gap-2 py-1.5 px-4 rounded-full bg-primary/10 border border-primary/30 backdrop-blur-md mb-6 animate-fade-in-up">
                <span class="w-2 h-2 rounded-full bg-primary animate-pulse"></span>
                <span class="text-primary text-xs font-bold tracking-widest uppercase">Featured Project</span>
              </div>
              
              <h2 class="text-5xl md:text-7xl lg:text-8xl font-black text-white mb-6 leading-[1.1] animate-fade-in-up delay-100 tracking-tight">
                {{ project.title }}
              </h2>
              
              <p class="text-lg md:text-xl text-surface-muted mb-10 leading-relaxed animate-fade-in-up delay-200 font-light max-w-xl">
                {{ project.description }}
              </p>
    ...
    <section id="intro" class="py-32 bg-background-soft border-b border-border relative overflow-hidden">
      <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] bg-primary/5 rounded-full blur-[120px]"></div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          ...
          >
             <div class="h-56 overflow-hidden relative">
               <img :src="p.image" class="w-full h-full object-cover group-hover:scale-110 transition duration-700 ease-out opacity-80 group-hover:opacity-100" />
               <div class="absolute inset-0 bg-background/40 group-hover:bg-transparent transition duration-500"></div>
             </div>
             <div class="p-8">
               <div class="flex justify-between items-center mb-4">
                 <span class="text-xs font-bold text-primary px-3 py-1 bg-primary/10 rounded-lg uppercase tracking-wider">{{ p.category }}</span>
                 <Icon name="heroicons:arrow-up-right" class="text-surface-muted group-hover:text-primary transition-colors" />
               </div>
               <h3 class="text-2xl font-bold text-white mb-3 group-hover:text-primary transition-colors">{{ p.title }}</h3>
    ...
    <section class="py-32 bg-background-soft relative overflow-hidden">
      ...
      <div class="container mx-auto px-6 relative z-10">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-6">
           ...
           >
             <img :src="item.image" class="w-full h-full object-cover opacity-60 group-hover:opacity-100 transition duration-500 grayscale group-hover:grayscale-0" />
             
             <div class="absolute inset-0 bg-gradient-to-t from-background via-transparent to-transparent opacity-80 group-hover:opacity-40 transition-opacity"></div>

             <div class="absolute inset-0 flex flex-col items-center justify-center opacity-0 group-hover:opacity-100 transition duration-300 p-4 text-center z-10">
                <div class="w-12 h-12 bg-primary rounded-full flex items-center justify-center text-white mb-2 shadow-lg scale-0 group-hover:scale-100 transition-transform duration-300 delay-100">
                  <Icon name="heroicons:eye" />
                </div>
                <h4 class="font-bold text-white text-lg translate-y-4 group-hover:translate-y-0 transition duration-300 delay-75">{{ item.title }}</h4>
             </div>
           </NuxtLink>
        </div>
      </div>
